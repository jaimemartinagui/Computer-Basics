...
<p><strong>Two's complement representation</strong></p>

We are going to use the **"Two's complement"** approach to address the binary subtraction. This way of representig binary numbers consist on the following:

- If the number is positive just take its standard binary representation and add a 0 bit at the beginning (e.g. 8 would be -> 01000).
...
The negative numbers can be generated by using the `NOT` gate and the `general_adder` we created before, as shown in the following diagram.

<div align="left">
  <img src="img/convert_to_negative.png" alt="convert_to_negative" height="300" width="800"/>
</div>

This format conversion can be done with the `convert_to_twos_complement_format` function in the `binary_subtraction.py` module.

The `general_subtractor` is then the same as the `general_adder` just by receiving the input values in the Two's complement format.
